...
class Craft(GameObject):
    ...
    def __init__(self, boundary: tuple):
        ...
        for i in range(14):
            ...
            off_x += 1
        ''' Setup actions '''
        self.actions.append(Action([self.frames[0]]))
        self.actions.append(Action([self.frames[1], self.frames[2]]))
        self.actions.append(Action([self.frames[3], self.frames[4]]))
        self.actions.append(Action([self.frames[2], self.frames[1], self.frames[0]]))
        self.actions.append(Action([self.frames[4], self.frames[3], self.frames[0]]))

    def set_input(self, input: Input) -> None:
        self.input = input

    async def update(self, time: int) -> None:
        if self.is_alive() is False:
            self.explosion.update(time)
            await self.update_bullets(time)
            return
    ...
    async def update_bullets(self, time: int) -> None:
